...
Character Player::getCharacter() { return playerCharacter; }
...
void Player::setBoost(float s) { speedBoost = s; }
...
void Player::setCharacter(int c) {
	...
	shade.load("Shadow.obj");
	glBindTexture(GL_TEXTURE_2D, shadow);
	shadow = ilutGLLoadImage("Shadow.png");
	glBindTexture(GL_TEXTURE_2D, tex);
	if (playerCharacter.getChar() == 1) {
		tex = ilutGLLoadImage("BrickMap.png");
		glBindTexture(GL_TEXTURE_2D, Goldtex);
		Goldtex = ilutGLLoadImage("GoldenBoy.png");
	}
	if (playerCharacter.getChar() == 2) {
		tex = ilutGLLoadImage("LynMap.png");
	}
	if (playerCharacter.getChar() == 3) {
		tex = ilutGLLoadImage("OzeanMap.png");
	}
	if (playerCharacter.getChar() == 4) {
		tex = ilutGLLoadImage("ZeitFace.png");
	}
...
glm::vec3 Player::getCoord() { return playerCoord; }
#define SMALL_NUM   0.00000001 
int
intersect3D_RayTriangle(glm::vec3 O, glm::vec3 D, glm::vec3 &len, Triangle T, glm::vec3 &I, float xOff)
{
...
void Stage::set(int t) {
	...
	stageModel.load("img/cell"+ std::to_string(type)+".obj");
	begin = stageModel.getBeg();
	end= stageModel.getEnd();
	glBindTexture(GL_TEXTURE_2D, tex);
	tex = ilutGLLoadImage("celltext.png");
	glBindTexture(GL_TEXTURE_2D, 0);
	
}
float Stage::getOff() { return offset+end+10.0f; }
...
Stage::~Stage() {}
int Stage::getTriSize() { return stageModel.getTSize(); }
Triangle Stage::getTriangle(int pos) { return stageModel.triangle(pos); }
void Stage::Draw() {
	if (draw) {
		glPushMatrix();
...
void Camera::load(float l) {
	for (int i = 0; i < l; i++) {
		...
		std::string line;
		std::ifstream file("img/cell" + std::to_string(i+1) + ".txt");
		if (file.fail()) {
			return;
		}
		while (!file.eof()) {
